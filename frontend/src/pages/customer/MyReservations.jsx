              ...
              <div key={r.reservation_id} className="card" style={{
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: '1rem' }}>
                  <div>
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: '0.5rem 2rem' }}>
                      ...
                      <div>
                        ...
                        <p style={{ margin: 0 }}>{new Date(r.reservation_date).toLocaleDateString('en-PH', { year: 'numeric', month: 'long', day: 'numeric' })}</p>
                      </div>
                      <div>
                        <span style={{ fontSize: '0.75rem', color: 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: '0.05em' }}>Intended Interment</span>
                        <p style={{ margin: 0 }}>{r.intended_use_date ? new Date(r.intended_use_date).toLocaleDateString('en-PH', { year: 'numeric', month: 'long', day: 'numeric' }) : '—'}</p>
                      </div>
                      <div>
                  ...
                  <div style={{
                    ...
                    <div style={{ fontSize: '1.25rem', fontWeight: 700, color: isPaid ? '#22c55e' : '#eab308' }}>
                      {isPaid ? '✓ Paid' : `₱${parseFloat(r.total_price).toLocaleString()}`}
                    </div>
                    {!isPaid && (
